...
const {readJson} = require('g11n-pipeline/lib/utils');

const flatten = require('g11n-pipeline-flatten');

function maybeFlatten(argv, d) {
  if(argv.flatten) {
...
function maybeExpand(argv, d) {
  if(argv.flatten) {
    return flatten.expand(d);
  } else {
    return d;
  }
}
...
module.exports = {
  ...
  import: async (client, argv) => client.bundle(argv.bundle)
    .uploadStrings({
      ...
      strings: maybeFlatten(argv, await readJson(argv.file))
    }),

  export: (client, argv) => client.bundle(argv.bundle)
    .getStrings({languageId: argv.languages})
    .then(r => maybeExpand(argv, r.resourceStrings)),
  /**
   * show trs
   */
  trs: (client, argv) => client.trs({status: argv.status, summary: false}),
  /**
  * show trs with summary only
  */
  trs_summary: (client, argv) => client.trs({status: argv.status, summary: true}),

  /**
   * show doctrs
   */
  docTrs: (client, argv) => client.docTrs({status: argv.status, summary: false}),
  /**
   * show doctrs with summary only
   */
  docTrs_summary: (client, argv) => client.docTrs({status: argv.status, summary: true})
